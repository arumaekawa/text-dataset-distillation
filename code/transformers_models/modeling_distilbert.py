...
class Embeddings(nn.Module):
    ...
    def forward_with_params(self, input_ids, weights=None):
        ...
        module_name_list = ["word_embeddings", "position_embeddings", "LayerNorm"]
...
class TransformerBlock(nn.Module):
    ...
    def forward_with_params(
        ...
        """
        assert weights is not None
        module_name_list = ["attention", "sa_layer_norm", "ffn", "output_layer_norm"]
        weights_dict = {module_name: OrderedDict() for module_name in module_name_list}
        for name, param in weights.items():
            module_name, param_name = name.split(".", maxsplit=1)
            weights_dict[module_name][param_name] = param
...
class Transformer(nn.Module):
    ...
    def forward_with_params(
        ...
        """
        assert weights is not None
        layer_weights = OrderedDict({i: OrderedDict() for i in range(self.n_layers)})
        for name, param in weights.items():
...
class DistilBertModel(DistilBertPreTrainedModel):
    ...
    def resize_position_embeddings(self, new_num_position_embeddings: int):
        ...
        logger.info(
            f"Setting `config.max_position_embeddings={new_num_position_embeddings}`transformers."
        )
